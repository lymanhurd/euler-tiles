"""
This class represents the 1's in the matrix.  The Nodes are formed into
circularly linked lists to make rows (i.e., joined by Left/Right and
vertically linked lists to make columns.  In addition, each node contains a
link to the column which contains it.  For the purposes of the algorithm,
rows all are treated symmetrically and therefore do not need to be tracked.
"""
...
class Node:
    ...
    # Iterators for traversing circular linked lists back to the starting
    # point.  In the form needed in the algorithm, these iterators never
    # return themselves but iterate over all other elements in the given
    # direction.
    def right_iter(self):
        node = self.right
        while node != self:
...
class Column(Node):
    def __init__(self, left=None, right=None, up=None, down=None, size=0,
                 name=None):
...
def create_columns(column_labels):
    columns = [Header(len(column_labels))] + [
        Column(name=label) for label in column_labels]
